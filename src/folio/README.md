...
Then autoload the `phpFolioClient` namespace (e.g. via Composer's PSR-4 autoloading
pointed at this directory), or `require` each file manually.
...
|---|---|---|
| [`FolioConfig`](FolioConfig.php) | Connection/credential settings, loaded from INI/array/object | — |
| [`FolioUtils`](FolioUtils.php) | UUID and JSON validation helpers | — |
| [`FolioLogger`](FolioLogger.php) | Tab-delimited query log, optional `error_log` mirroring | — |
| [`FolioAuth`](FolioAuth.php) | Login/token lifecycle (RTR), tracks access-token expiry | `FolioConfig` |
| [`FolioInformation`](FolioInformation.php) | Read-only tenant/environment metadata | `FolioConfig`, `FolioAuth` |
| [`FolioClient`](FolioClient.php) | Core HTTP client: GET/PUT/PATCH/POST/DELETE + pagination | `FolioConfig`, `FolioAuth`, `FolioUtils`, `FolioLogger`?, `FolioInformation`? |
| [`FolioFileHandler`](FolioFileHandler.php) | Binary file upload/download | `FolioClient` |
| [`FolioDataExport`](FolioDataExport.php) | Orchestrates the data-export workflow | `FolioClient`, `FolioFileHandler` |
| [`FolioReferenceDataManager`](FolioReferenceDataManager.php) | Reference/control-vocabulary lookups (locations, material types, patron groups, etc.) | `FolioClient` |
